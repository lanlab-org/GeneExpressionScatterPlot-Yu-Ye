(function(e) {
	...
	function z(a, b) {
		...
		e.each(b, function(d, c) {
			if (e.isPlainObject(c)) {
				...
				u && (-1 === b.indexOf("<?xml") && (b = '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + b), b = b.replace(
	...
	if ("undefined" === typeof f || "undefined" !== typeof navigator && /MSIE [1-9]\./.test(navigator.userAgent)) r =
		...
		var w = f.document.createElementNS("http://www.w3.org/1999/xhtml", "a"),
			...
			E = function(a, b, d) {
				...
				if (K) e = (f.URL || f.webkitURL || f).createObjectURL(a), setTimeout(function() {
					...
					g.readAsDataURL(a);
					c.readyState = c.INIT
				} else e || (e = (f.URL || f.webkitURL || f).createObjectURL(a)), d ? f.location.href = e : f.open(e, "_blank") ||
					(f.location.href = e), c.readyState = c.DONE, h(), C(e)
			},
			k = E.prototype;
		"undefined" !== typeof navigator && navigator.msSaveOrOpenBlob ? r = function(a, b, d) {
			...
			k.onwrite = k.onabort = k.onerror = k.onwriteend = null, r = function(a, b, d) {
	...
	var N = function(a) {
		...
		G = function(a, b) {
			for (var d = F(b), c = a.buttons.exportData(b.exportOptions), e = b.fieldBoundary,
					h = b.fieldSeparator, g = RegExp(e, "g"), f = b.escapeChar !== q ? b.escapeChar : "\\", i = function(a) {
						for (var b = "", c = 0, d = a.length; c < d; c++) 0 < c && (b += h), b += e ? e + ("" + a[c]).replace(g, f + e) +
							e : a[c];
						return b
					}, l = b.header ? i(c.header) + d : "", j = b.footer && c.footer ? d + i(c.footer) : "", n = [], m = 0, k = c.body
					.length; m < k; m++) n.push(i(c.body[m]));
			return {
				str: l + n.join(d) + j,
				rows: n.length
			}
		...
		H = function() {
			if (!(-1 !== navigator.userAgent.indexOf("Safari") && -1 === navigator.userAgent.indexOf("Chrome") && -1 ===
					navigator.userAgent.indexOf("Opera"))) return !1;
	...
	p.ext.buttons.copyHtml5 = {
		...
		action: function(a, b, d, c) {
			...
			k = e("<span>" + b.i18n("buttons.copyKeys",
	...
	p.ext.buttons.csvHtml5 = {
		bom: 1,//CSV直接导出的是UTF-8,而Excel会尝试用GBK编码解析，但是如果用UTF-8+BOM的方式导出,Excel则会以UTF-8编码解析
			...
			var f = this,
				...
				l, p, j = function(a) {
					...
					for (var b = 0, d = a.length; b < d; b++) {
						...
						f || ("number" === typeof a[b] || a[b].match && a[b].match(/^-?\d+(\.\d+)?$/) && !a[b].match(/^0\d+/) ? f = o(
							g, "c", {
								attr: {
									t: "n",
									r: i
